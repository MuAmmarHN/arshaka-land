...
func main() {
	...
	// Enable all QR codes
	result, err := db.Exec("UPDATE qr_code SET enable = true")
	if err != nil {
		log.Fatal("Error enabling QR codes:", err)
	}
	...
	fmt.Printf("Successfully enabled %d QR codes\n", rowsAffected)
	...
	fmt.Printf("Total enabled QR codes: %d\n", count)
}
